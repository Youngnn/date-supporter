<template>
  <div id="app">
        ...
        <v-card>
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12>
                  <v-text-field label="순위" v-model="deviceInfo.deviceId" required></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field label="사용자"  v-model="deviceInfo.name" required></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field label="여행지"  v-model="deviceInfo.spot" required></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field label="참고링크"  v-model="deviceInfo.link" required></v-text-field>
                </v-flex> 
                <v-flex xs12>
                  <v-text-field label="여행지 위도"  v-model="deviceInfo.latitude" required></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field label="여행지 경도"  v-model="deviceInfo.longitude" required></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field label="설명"  v-model="deviceInfo.description" required></v-text-field>
                </v-flex> 
                <v-flex xs12>
                  <v-text-field label="등록날자"  v-model="deviceInfo.register_date" required></v-text-field>
                </v-flex> 
              </v-layout>
            </v-container>
          </v-card-text>
	...
	<v-btn color="primary" v-on:click.native="addContact()">코스 추가</v-btn>
	<v-data-table :headers="headers" :items="items" class="elevation-1">
		...
		<tr>
			...
			<td class="text-xs-right">
				...
				<v-btn color="primary" v-on:click.native="sendEvent(row.item)">검색</v-btn>
			</td>
		</tr>
      </template>
	</v-data-table>
...
export default {
  data () {
    return {
      urlinfo:'http://localhost:8080/devices',
      deviceInfo:{
        _id: null,
        deviceId: null,
        name:null,
      ...
      headers: [
        { text: '순위', align: 'left', value: 'deviceId'},
        { text: '사용자', align: 'left', value: 'name'},
        { text: '여행지', align: 'left', value: 'spot'},
        { text: '참고링크', align: 'left', value: 'link'},
        { text: '여행지 위도', align: 'left',value: 'latitude'},
        { text: '여행지 경도', align: 'left',value: 'longitude'},
        { text: '설명', align: 'left', value: 'description'},
		{ text: '등록날자', align: 'left', value: 'register_date'},
  ...
  methods:{
    ...
    updateContact(data){
      ...
      this.deviceInfo.spot = data.spot;
      this.deviceInfo.link = data.link;
    ...
    btnClick($event){
      ...
      if($event.target.innerHTML == "확인"){
        if(this.dialogTitle =="추가"){ 
          axios.post(this.urlinfo,{ 
            deviceId: this.deviceInfo.deviceId, name: this.deviceInfo.name, spot: this.deviceInfo.spot, link: this.deviceInfo.link, 
            latitude:  this.deviceInfo.latitude, longitude: this.deviceInfo.longitude, description:  this.deviceInfo.description, 
      ...
      this.deviceInfo.spot = null;
      this.deviceInfo.link = null;
